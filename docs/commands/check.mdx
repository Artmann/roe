...
analyses. Set them in a [config file](/configuration), which the combined run
reads exactly once, or run the [individual command](/commands/dead-code) when you
want to tune one analysis for a single invocation.
