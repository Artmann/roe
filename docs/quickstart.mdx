---
title: "Quickstart"
description: "Run your first dead-code and duplicate scan in two minutes."
icon: "bolt"
---

Both roe commands take a path to a directory, a `.sln` file, or a `.csproj`
file, and default to the current directory. There's nothing to configure up
...
<Steps>
  ...
  <Step title="Wire it into scripts or CI">
    Both commands exit `0` when clean, `1` when they have findings to
    report, and `2` on error, and both support `--format json` for a stable,
  ...
  <Step title="Tune out false positives">
    ...
    [`roe.json` config file](/configuration) to ignore whole directories and
    declare extra entry points.
  </Step>
</Steps>
