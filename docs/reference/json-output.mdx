...
| `version` | Schema version, `1`. |
| `root` | The analysis root path. |
| `summary` | Scan totals: `projects`, `filesScanned`, `symbols`, one count per check (`highComplexity`, `highCognitiveComplexity`, `longMethods`, `tooManyParameters`, `largeFiles`, `largeTypes`, `circularDependencies`), and `elapsedMs`. `commitsWalked` is present only when `--hotspots` was passed. |
...
| `findings[].metric` | The measured value — the complexity score, line count, parameter count, or member count. |
